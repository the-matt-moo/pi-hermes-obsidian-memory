...
export function searchSessionAnchors(
  ...
  try {
    canonicalSessionsDir = fs.realpathSync(options.sessionsDir);
  } catch {
    return { success: false, ranges: [], message: `sessionsDir cannot be resolved: ${options.sessionsDir}` };
  }
  const files = findJsonlFiles(canonicalSessionsDir).sort();
...
function findJsonlFiles(root: string): string[] {
  ...
  const walk = (dir: string): void => {
    ...
    for (const entry of entries) {
      ...
      const candidateRelative = path.relative(canonicalRoot, canonicalCandidate);
      if (candidateRelative === ".." || candidateRelative.startsWith(`..${path.sep}`) || path.isAbsolute(candidateRelative)) continue;
      let candidateStat: fs.Stats;
      try {
        candidateStat = fs.statSync(canonicalCandidate);
      } catch {
        continue;
      }
      if (candidateStat.isDirectory()) {
        walk(canonicalCandidate);
      } else if (candidateStat.isFile() && entry.name.endsWith(".jsonl")) {
