...
const SECRET_PATTERNS: Array<{ pattern: RegExp; id: string; severity: "high" | "medium" }> = [
  ...
  { pattern: /\bsk_live_[0-9A-Za-z]{20,}\b/, id: "stripe_live_secret_key", severity: "high" },
  // Tokens
  ...
  { pattern: /["']type["']\s*:\s*["']service_account["']/i, id: "gcp_service_account", severity: "high" },
  { pattern: /(?:[?&])sv=\d{4}-\d{2}-\d{2}&[^\s#]{0,600}(?:^|&)sig=/i, id: "azure_sas_token", severity: "high" },
  { pattern: /\bAC[0-9a-f]{32}\b/, id: "twilio_account_sid", severity: "high" },
  { pattern: /\b(?:twilio[_-]?)?auth[_-]?token\s*[=:]\s*[0-9a-f]{32}\b/i, id: "twilio_auth_token", severity: "high" },
  { pattern: /\bSK[0-9a-f]{32}\b/, id: "twilio_api_key", severity: "high" },
  // SSH keys
