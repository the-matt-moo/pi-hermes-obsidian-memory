...
function fail(message) {
  ...
  console.error("  cd pi-hermes-obsidian-memory && npm install");
...
if (mode === "check") {
  ...
  if (!existsSync(join(root, "tests")) || !existsSync(join(root, "scripts", "run-tests.mjs"))) {
    fail(
      "`npm test` requires the tests/ directory and scripts/run-tests.mjs. The published npm package intentionally omits tests.",
    );
  }
} else {
